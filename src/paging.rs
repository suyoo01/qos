...
pub fn get_page_table() -> &'static mut L1PageTable {
    let kern_pgdir_addr = (crate::reg::TTBR0::read() & 0xffffc000) as usize;
...
use crate::mem::{page_alloc, fn_to_pa, pa_to_va};
...
pub fn map_va_to_fn(va: usize, frame_number: usize, flag: usize) {
    ...
    let directory = &mut page_table.entries[va&DIRECTORY_MASK];
    ...
    if directory.is_section() {
        ...
        *directory = L1TableEntry{ data: fn_to_pa(page_alloc(1)) | 0x1};
    }
    let l2_table = directory.get_l2_table();
    l2_table.entries[(pa&!DIRECTORY_MASK)/PAGE_SIZE] = L2TableEntry { data: L2_ADDR_MASK&pa | flag | 0x89e};
}
